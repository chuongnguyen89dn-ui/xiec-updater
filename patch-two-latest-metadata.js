...
async function getLargeFile(f) {
  ...
  if (!f.sha) throw new Error('Target file has neither content nor sha');
  const blobUrl = `https://api.github.com/repos/${OWNER_REPO}/git/blobs/${f.sha}`;
  const br = await fetch(blobUrl, { headers });
...
async function main() {
  ...
  const movies = JSON.parse(await getLargeFile(f));

  const patches = {
    'SNOS-357': {
      releaseInfo: '2026-08-07', year: '2026', runtime: '1 giờ 58 phút', director: ['Inaball'],
      actors: ['Niko Kawagoe'],
      actor_photos: { 'Niko Kawagoe': 'https://commons.wikimedia.org/wiki/Special:Redirect/file/Niko_Kawagoe.jpg' },
      metadata_sources: ['Subtitle Nexus', 'Wikimedia Commons'], metadata_verified: true
    },
    'MIKR-084': {
      releaseInfo: '2026-04-17', year: '2026', studio: 'MOODYZ', maker: 'MOODYZ',
  ...
  for (const m of movies) {
    ...
    for (const [k, v] of Object.entries(p)) {
      ...
      } else if (k === 'actor_photos') {
        const merged = { ...(m.actor_photos || {}), ...v };
        if (JSON.stringify(m.actor_photos || {}) !== JSON.stringify(merged)) { m.actor_photos = merged; changed++; }
  ...
  const u = await fetch(api, { method: 'PUT', headers: { ...headers, 'Content-Type': 'application/json' }, body: JSON.stringify({ message: 'Add verified metadata and actor photos for SNOS-357 and MIKR-084', content, sha: f.sha, branch: 'main' }) });
